<?php
                    ...
                    if(isset($_GET['p_id']) && $_GET['p_id']!=""){
                        ...
                        $level1Uplink = $data['upclassid'];
                        $level1=$data['uplevel'];
                        $levelOpen = '<li class="breadcrumb-item"><a href="drugstore.php?classid='.$level1Upclassid.'&level='.$level1.'">'.$level1Cname .'</a></li>';
                        //處理第二層
                        $level2Cname=$data['cname'];
                        $level2Classid=$data['classid'];
                        $level2Open='<li class="breadcrumb-item"> <a href="drugstore.php?classid='.$level2Classid.'">'.
                        $level2Cname.'</a></li>';
                        //處理第三層(產品名稱)
                        ...
                        //選擇第一層類別
                        $SQLstring = sprintf("SELECT * FROM pyclass WHERE level=%d", $_GET['level'], $_GET['classid']);
                        $classid_rs = $link->query($SQLstring);
                        $data = $classid_rs->fetch();
